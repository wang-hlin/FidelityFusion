...
from utils import arg_parse, model_dict, normalize_data, find_matching_indices
...
def get_testing_data_hpo(data_root, benchmark="lcbench", task_name="airlines"):
    data_dir = os.path.join(data_root, "{}-{}".format(benchmark, task_name))
    # data_files = os.listdir(data_dir)

    hyperparameter_index = pd.read_csv("{}/hyperparameter_index.csv".format(data_dir))
    objectives_evaluations = pd.read_csv("{}/final_data.csv".format(data_dir))
    ...
    tiny_objectives_default = objectives_evaluations
    ...
    for _i in range(tiny_train_y.shape[1]):  # np_array.shape[1] is 100
        # Extract the i-th column and convert it to a PyTorch tensor of shape (1000, )
        col_tensor = torch.tensor(tiny_train_y.iloc[:, _i])

        # Reshape the tensor to (1, 1000) and add it to the list
        tensor_y_list.append(col_tensor.unsqueeze(1))

    n_samples = tiny_objectives_default.shape[0]

    # n_samples = tiny_objectives_default.shape[0]
    #
    # tr_x = torch.tensor(tensor_tiny_train_x[:n_samples//2, ...]).float()
    # eval_x = torch.tensor(tensor_tiny_train_x[n_samples//2:, ...]).float()
    # tr_y_list = [y[:n_samples//2, ...].float() for y in tensor_y_list]
    # eval_y_list = [y[n_samples//2:, ...].float() for y in tensor_y_list]
    #
    # # tr_x_sliced = tr_x[:, :50]
    ...
    n_fidelity = tiny_objectives_default.shape[1]
    return tensor_tiny_train_x, tensor_tiny_train_x, tensor_y_list, tensor_y_list, n_samples, n_fidelity
...
#     normalized_tr_x_list = [[(t - mean) / std for t in tr_x]
#                             for tr_x, mean, std in zip(tr_x_list, mean_tr_x, std_tr_x)]
#
...
if __name__ == '__main__':
    ...
    tr_x, eval_x, tr_y_list, eval_y_list, sample_num, fidelity_num = get_testing_data_hpo(data_root, benchmark, task)
    ################################
    normalized_x = F.softmax(tr_x, dim=1)
    entropy = -torch.sum(normalized_x * torch.log(normalized_x), dim=1)
    # pick top 10% of entropy
    num_rows_to_select = int(cfg.DATASET.TRAIN_RATIO * sample_num)
    ...
    ##############################
    ground_truth = eval_y_list[-1]
    evaluation_num = len(ground_truth)
    print(len(tr_y_list))
    ...
    for e in range(len(stopping_trail)):
        ...
        # ground_truth = norm_tool.denormalize_output(eval_y_list[-1], len(tr_y_list)-1)
        predict_y = norm_tool.denormalize_output(predict_y, len(tr_y_list) - 1)
        # plot_container([ground_truth.reshape(-1, *src_y_shape), predict_y.reshape(-1, *src_y_shape)],
        #                ['ground truth', 'predict'], 0).plot(3)
        print("model_name:", model_name)
        matching_gt_indices = find_matching_indices(full_x, tr_x[current_trail - 1])
        evaluation_num = len(ground_truth)
        sorted_predict_y, sorted_predict_y_indices = torch.sort(predict_y, dim=0)
        sorted_ground_truth, sorted_ground_truth_indices = torch.sort(ground_truth[matching_gt_indices], dim=0)
        ...
        #                                 ground_truth=sorted_ground_truth_indices,
        #                                 k=top_k)
        #     print(f"Overlap Precision @{k}%: {similarity}")
        #
        # mse = torch.mean((ground_truth[matching_gt_indices] - predict_y) ** 2)
        # print("MSE:", mse)

        from scipy.stats import spearmanr
        from scipy.stats import kendalltau

        # Assuming ground_truth and predict_y are your rank vectors
        spearman_correlation, _ = spearmanr(ground_truth[matching_gt_indices], predict_y)
        print("Spearman correlation:", spearman_correlation)

        # Assuming ground_truth and predict_y are your rank vectors
        kendall_tau, _ = kendalltau(ground_truth[matching_gt_indices], predict_y)

        print(f"Kendall's Tau coefficient: {kendall_tau}")

        # return the top_k hyperparameters index, prepare for next round of evaluation
        top_k_hyperparameters_index = find_matching_indices(ground_truth, sorted_predict_y[:select_top_k, :])
