    ...
    for f in range(CARmodel.fidelity_num):
        ...
        if f == 0:
            ...
            for i in range(max_iter):
                ...
                if i == max_iter - 1:
                    data_manager.add_data(fidelity_index=-f, raw_fidelity_name='res-{}'.format(f), x=subset_x,
                                          y=y_residual)
...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.errorbar(x_test.flatten(), ypred.reshape(-1).detach(), ypred_var.diag().sqrt().squeeze().detach(), fmt='r-.',
                 alpha=0.2)
    plt.fill_between(x_test.flatten(), ypred.reshape(-1).detach() - ypred_var.diag().sqrt().squeeze().detach(),
                     ypred.reshape(-1).detach() + ypred_var.diag().sqrt().squeeze().detach(), alpha=0.2)
